...
static int Demoasic(Frame *frame, const IspPrms *isp_prm)
{
    ...
    FOR_ITER(h, frame->info.height)
    {
        FOR_ITER(w, frame->info.width)
        {
			...
			//if (raw32_in[pixel_idx] > max) {
            //    max = raw32_in[pixel_idx];
            //}
	...
	//		for (int i = 0; i < 3; ++i) {
	//			if ((bgr_out[3 * pixel_idx + i] > 256) || (bgr_out[3 * pixel_idx + i] < 0)) {
	//				std::cout << "error " << w << " " << h << " " << bgr_out[3 * pixel_idx + i]  << "\n";
	//			}
	//		}
	//	}
	//
	...
	//std::cout << max << "\n";
	//std::cout << frame->data.bgr_s32_o << " | " << frame->data.bgr_s32_i << "\r\n";
	//std::cout << "isp max " << isp_prm->info.max_val << "\n";
