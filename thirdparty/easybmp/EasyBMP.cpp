...
ebmpBYTE BMP::FindClosestColor(RGBApixel &input)
{
    ...
    while (i < NumberOfColors)
    {
        ...
        int TempMatch = IntSquare((int)Attempt.Red - (int)input.Red) + IntSquare((int)Attempt.Green - (int)input.Green) + IntSquare((int)Attempt.Blue - (int)input.Blue);
...
bool Rescale(BMP &InputImage, char mode, int NewDimension)
{
    ...
    for (int j = 0; j < NewHeight - 1; j++)
    {
        ...
        for (int i = 0; i < NewWidth - 1; i++)
        {
            ...
            InputImage(i, j)->Red = (ebmpBYTE)((1.0 - ThetaI - ThetaJ + ThetaI * ThetaJ) * (OldImage(I, J)->Red) + (ThetaI - ThetaI * ThetaJ) * (OldImage(I + 1, J)->Red) + (ThetaJ - ThetaI * ThetaJ) * (OldImage(I, J + 1)->Red) + (ThetaI * ThetaJ) * (OldImage(I + 1, J + 1)->Red));
            InputImage(i, j)->Green = (ebmpBYTE)((1.0 - ThetaI - ThetaJ + ThetaI * ThetaJ) * OldImage(I, J)->Green + (ThetaI - ThetaI * ThetaJ) * OldImage(I + 1, J)->Green + (ThetaJ - ThetaI * ThetaJ) * OldImage(I, J + 1)->Green + (ThetaI * ThetaJ) * OldImage(I + 1, J + 1)->Green);
            InputImage(i, j)->Blue = (ebmpBYTE)((1.0 - ThetaI - ThetaJ + ThetaI * ThetaJ) * OldImage(I, J)->Blue + (ThetaI - ThetaI * ThetaJ) * OldImage(I + 1, J)->Blue + (ThetaJ - ThetaI * ThetaJ) * OldImage(I, J + 1)->Blue + (ThetaI * ThetaJ) * OldImage(I + 1, J + 1)->Blue);
        }
...
bool WriteBgrMemToBmp(const char* file, char* bgr, int width, int height, int bpp)
{
    if (!file || !bgr) {
        return false;
    }
    ...
    out_file.SetBitDepth(bpp);
    out_file.SetSize(width, height);

    for (int idy = 0; idy < height; ++idy)
    {
        for (int idx = 0; idx < width; ++idx)
        {
            int pixel_id = idx + idy * width;

            RGBApixel pixel;
            pixel.Blue = bgr[3 * pixel_id + 0];
            pixel.Green = bgr[3 * pixel_id + 1];
            pixel.Red = bgr[3 * pixel_id + 2];
            pixel.Alpha = 0;

            out_file.SetPixel(idx, idy, pixel);
        }
    }

    return out_file.WriteToFile(file);
}